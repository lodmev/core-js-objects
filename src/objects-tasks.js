...
function makeWord(lettersObject) {
  ...
  for (let i = 0; i < objEntries.length; i += 1) {
    const [letter, positions] = objEntries[i];
    for (let j = 0; j < positions.length; j += 1) {
      result[positions[j]] = letter;
    }
  }
  return result.join('');
}
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i] > 25 && queue[i] - 25 > cashbox) {
      return false;
    }
    cashbox -= queue[i] - 25;
    cashbox += 25;
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((item) => {
    const [key, value] = [keySelector(item), valueSelector(item)];
    if (result.has(key)) {
      result.get(key).push(value);
    } else {
      result.set(key, Array(value));
    }
  });
  return result;
}
